    ...
    void create_new_mesh(int cx, int cz)
    {
        ...
        if (chunks.Contains(new Vector2(cx - 1, cz)))
        {
            ...
            for (int i = 1; i < zCap; i++)
            {
                ...
                myMeshNorms[s] = avgNormal;
                myOldMeshNorms[j] = avgNormal;
            }
            mesh.normals = myMeshNorms;
            oldMesh.normals = myOldMeshNorms;
        }
        if (chunks.Contains(new Vector2(cx + 1, cz)))
        {
            ...
            for (int i = 1; i < zCap; i++)
            {
                int j = i * (zCap + 1);
                int jUp = j - (zCap + 1);
                int jUpRight = j - (zCap + 1) + 1;
                int jRight = j + 1;
                int jDownRight = j + (zCap + 1) + 1;
                int s = i * (zCap + 1) + zCap;
                int sUp = s - (zCap + 1);
                int sUpLeft = s - (zCap + 1) - 1;
                int sLeft = s - 1;
                int sDownLeft = s + (zCap + 1) - 1;


                Vector3 tri1edge1 = mesh.vertices[sUp] - mesh.vertices[s];
                Vector3 tri1edge2 = mesh.vertices[sUpLeft] - mesh.vertices[s];
                var normal1 = Vector3.Cross(tri1edge1, tri1edge2).normalized;

                Vector3 tri2edge1 = mesh.vertices[sUpLeft] - mesh.vertices[s];
                Vector3 tri2edge2 = mesh.vertices[sLeft] - mesh.vertices[s];
                var normal2 = Vector3.Cross(tri2edge1, tri2edge2);

                Vector3 tri3edge3 = mesh.vertices[sLeft] - mesh.vertices[s];
                Vector3 tri3edge4 = mesh.vertices[sDownLeft] - mesh.vertices[s];
                var normal3 = Vector3.Cross(tri3edge3, tri3edge4).normalized;

                Vector3 tri4edge1 = oldMesh.vertices[jUpRight] - oldMesh.vertices[j];
                Vector3 tri4edge2 = oldMesh.vertices[jUp] - oldMesh.vertices[j];
                var normal4 = Vector3.Cross(tri4edge1, tri4edge2).normalized;

                Vector3 tri5edge1 = oldMesh.vertices[jRight] - oldMesh.vertices[j];
                Vector3 tri5edge2 = oldMesh.vertices[jUpRight] - oldMesh.vertices[j];
                var normal5 = Vector3.Cross(tri5edge1, tri5edge2).normalized;

                Vector3 tri6edge1 = oldMesh.vertices[jDownRight] - oldMesh.vertices[j];
                Vector3 tri6edge2 = oldMesh.vertices[jRight] - oldMesh.vertices[j];
                var normal6 = Vector3.Cross(tri6edge1, tri6edge2).normalized;
        ...
        if (chunks.Contains(new Vector2(cx, cz + 1)))
        {
            print("(" + cx + " , " + cz + ")" + "new mesh has top neighbor");
            Mesh oldMesh = chunks_and_tris[new Vector2(cx, cz + 1)];
            ...
            for (int i = 1; i < xCap; i++)
            {
                Vector3 tri1edge1 = mesh.vertices[xCap * (xCap + 1) + i + 1] - mesh.vertices[xCap * (xCap + 1) + i];
                Vector3 tri1edge2 = mesh.vertices[(xCap - 1) * (xCap + 1) + i + 1] - mesh.vertices[xCap * (xCap + 1) + i];
                var normal1 = Vector3.Cross(tri1edge1, tri1edge2).normalized;

                Vector3 tri2edge1 = mesh.vertices[(xCap - 1) * (xCap + 1) + i + 1] - mesh.vertices[xCap * (xCap + 1) + i];
                Vector3 tri2edge2 = mesh.vertices[(xCap - 1) * (xCap + 1) + i] - mesh.vertices[xCap * (xCap + 1) + i];
                var normal2 = Vector3.Cross(tri2edge1, tri2edge2);

                Vector3 tri3edge3 = mesh.vertices[(xCap - 1) * (xCap + 1) + i] - mesh.vertices[xCap * (xCap + 1) + i];
                Vector3 tri3edge4 = mesh.vertices[xCap * (xCap + 1) + i - 1] - mesh.vertices[xCap * (xCap + 1) + i];
                var normal3 = Vector3.Cross(tri3edge3, tri3edge4).normalized;

                Vector3 tri4edge1 = oldMesh.vertices[i - 1] - oldMesh.vertices[i];
                Vector3 tri4edge2 = oldMesh.vertices[i + xCap + 1] - oldMesh.vertices[i];
                var normal4 = Vector3.Cross(tri4edge1, tri4edge2).normalized;

                Vector3 tri5edge1 = oldMesh.vertices[i + xCap + 1] - oldMesh.vertices[i];
                Vector3 tri5edge2 = oldMesh.vertices[i + xCap + 2] - oldMesh.vertices[i];
                var normal5 = Vector3.Cross(tri5edge1, tri5edge2).normalized;

                Vector3 tri6edge1 = oldMesh.vertices[i + xCap + 2] - oldMesh.vertices[i];
                Vector3 tri6edge2 = oldMesh.vertices[i + 1] - oldMesh.vertices[i];
                var normal6 = Vector3.Cross(tri6edge1, tri6edge2).normalized;

                Vector3 avgNormal = ((normal1 + normal2 + normal3 + normal4 + normal5 + normal6) / 6).normalized;
                myMeshNorms[xCap * (xCap + 1) + i] = avgNormal; 
                myOldMeshNorms[i] = avgNormal;                 
            }
            mesh.normals = myMeshNorms;
            oldMesh.normals = myOldMeshNorms;
        }
